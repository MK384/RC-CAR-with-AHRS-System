...
SD_MPU6050_Result SD_MPU6050_Init(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Device DeviceNumber, SD_MPU6050_Accelerometer AccelerometerSensitivity, SD_MPU6050_Gyroscope GyroscopeSensitivity)
{
	...
	//------------------  [ clear calibration values ] --------------------------
		...
		DataStruct->Acce_Calibrate[0] = 0.0f;
		DataStruct->Acce_Calibrate[1] = 0.0f;
		DataStruct->Acce_Calibrate[2] = 0.0f;


		DataStruct->Gyro_Calibrate[0] = 0.0f;
		DataStruct->Gyro_Calibrate[1] = 0.0f;
		DataStruct->Gyro_Calibrate[2] = 0.0f;
...
SD_MPU6050_Result SD_MPU6050_SetAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Accelerometer AccelerometerSensitivity)
{
	...
	/* Config accelerometer */
	if(HAL_I2C_Master_Transmit(Handle, (uint16_t)address,&regAdd, 1, 1000) != HAL_OK)
	{
				return SD_MPU6050_Result_Error;
	}
...
SD_MPU6050_Result SD_MPU6050_SetLowPassFilter(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct, SD_MPU6050_Bandwidth bw){
	...
	I2C_HandleTypeDef* Handle = I2Cx;
	uint8_t address = DataStruct->Address;
	uint8_t regAdd =(uint8_t ) MPU6050_CONFIG;
...
SD_MPU6050_Result SD_MPU6050_ReadAccelerometer(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	if(HAL_I2C_Master_Receive(Handle, (uint16_t)address, data, 6, 1000) != HAL_OK)
		return SD_MPU6050_Result_DeviceNotConnected;
...
SD_MPU6050_Result SD_MPU6050_ReadAll(I2C_HandleTypeDef* I2Cx,SD_MPU6050* DataStruct)
{
	...
	result = SD_MPU6050_ReadAccelerometer(I2Cx, DataStruct);
	if (result != SD_MPU6050_Result_Ok ) return result;
	result = SD_MPU6050_ReadGyroscope(I2Cx, DataStruct);
	if (result != SD_MPU6050_Result_Ok ) return result;
	result = SD_MPU6050_ReadTemperature(I2Cx, DataStruct);
	if (result != SD_MPU6050_Result_Ok ) return result;
